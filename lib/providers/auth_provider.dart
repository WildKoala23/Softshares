...
class AuthProvider with ChangeNotifier {
  ...
  Future login(String email, String password) async {
    ...
    //Get city
    User user = await api.getUserLogged(_id);
    _user = user;
    //print('USER ID: $_id');
    // Store the JWT token
    // Save user data securely
    await storage.write(key: 'logged_user', value: user.toString());
    // await storage.write(key: 'userId', value: user.user_id);
    // await storage.write(key: 'firstName', value: user._firstname);
    // await storage.write(key: 'lastName', value: user._lastName);
    // await storage.write(key: 'email', value: user._email);

    // Load areas and cities data
    await _loadAreasAndCities();
    notifyListeners();
    return token;
  }
  ...
  Future<void> _loadAreasAndCities() async {
